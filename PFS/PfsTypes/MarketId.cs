...
public enum MarketId : int
{
    Unknown,
    CLOSED, // Allows to keep track old stock, but not anymore fetching its eods
    NASDAQ,
    NYSE,
    AMEX,
    TSX,
 ...
 * - Function 'StockClose' handles actual zero profit holding -> trade conversion,
 *   making all left overholdings on time of stock closed to be gone but not lost
 * - Actual moving stock to MarketId.CLOSED needs to be done manually under tracking's
 * - Simple case is where no holdings left, so just moving stock CLOSED special market,
 *   that gives it some special handling and doesnt attempt to get EOD for it anymore
 * - As importing and stock already done there is often cases where dont have EOD anymore,
 *   but still stock needs to be shown on many reports
 * - Special effort done for following:
 *      - StockMgmt.History, to show name changes and closing info
 *      - PfStocks, to show that its closed symbol
 * - Note! There isnt atm way to cancel operation nor delete closed stock if has 
 *   holdings/divs/trades... can add later if need raises..
 */

public static partial class Extensions
{
    public static bool IsReal(this MarketId marketId)
    {
        if (marketId == MarketId.Unknown || marketId == MarketId.CLOSED )
            return false;
...
#if false   // https://api.marketstack.com/v1/exchanges?access_key=
